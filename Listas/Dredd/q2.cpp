...
class lista{
	...
	public:
		...
		void removeInicio();
		void removeFim();
		void removeNome(string nomeEspecifico);
	...
	void lista::removeInicio()
	{
		...
		void lista::removeNome(string nomeEspecifico){
			noh* aux=primeiro;
			noh* anterior =NULL;
			
			while(aux != NULL && aux->informacoes.nomeEquipe != nomeEspecifico ){
				anterior =aux;
				aux=aux -> proximo;
